class TourismAngadiExecutor {

    public static void main(String[] tourism) {

        /*TourismAngadi t1 = new TourismAngadi();
        t1.tourismId = 101;
        t1.touristName = "Rahul";
        t1.email = "rahul@gmail.com";
        Package p1 = new Package();
        p1.packageId = 201;
        p1.destination = "Goa";
        p1.price = 15000;
        t1.pkg = p1;
        t1.displayTourismInfo();

        TourismAngadi t2 = new TourismAngadi();
        t2.tourismId = 102;
        t2.touristName = "Anita";
        t2.email = "anita@gmail.com";
        Package p2 = new Package();
        p2.packageId = 202;
        p2.destination = "Kerala";
        p2.price = 18000;
        t2.pkg = p2;
        t2.displayTourismInfo();

        TourismAngadi t3 = new TourismAngadi();
        t3.tourismId = 103;
        t3.touristName = "Kiran";
        t3.email = "kiran@gmail.com";
        Package p3 = new Package();
        p3.packageId = 203;
        p3.destination = "Manali";
        p3.price = 22000;
        t3.pkg = p3;
        t3.displayTourismInfo();

        TourismAngadi t4 = new TourismAngadi();
        t4.tourismId = 104;
        t4.touristName = "Sneha";
        t4.email = "sneha@gmail.com";
        Package p4 = new Package();
        p4.packageId = 204;
        p4.destination = "Ooty";
        p4.price = 12000;
        t4.pkg = p4;
        t4.displayTourismInfo();

        TourismAngadi t5 = new TourismAngadi();
        t5.tourismId = 105;
        t5.touristName = "Arjun";
        t5.email = "arjun@gmail.com";
        Package p5 = new Package();
        p5.packageId = 205;
        p5.destination = "Kashmir";
        p5.price = 30000;
        t5.pkg = p5;
        t5.displayTourismInfo();

        TourismAngadi t6 = new TourismAngadi();
        t6.tourismId = 106;
        t6.touristName = "Meera";
        t6.email = "meera@gmail.com";
        Package p6 = new Package();
        p6.packageId = 206;
        p6.destination = "Coorg";
        p6.price = 10000;
        t6.pkg = p6;
        t6.displayTourismInfo();

        TourismAngadi t7 = new TourismAngadi();
        t7.tourismId = 107;
        t7.touristName = "Ravi";
        t7.email = "ravi@gmail.com";
        Package p7 = new Package();
        p7.packageId = 207;
        p7.destination = "Mysore";
        p7.price = 9000;
        t7.pkg = p7;
        t7.displayTourismInfo();

        TourismAngadi t8 = new TourismAngadi();
        t8.tourismId = 108;
        t8.touristName = "Priya";
        t8.email = "priya@gmail.com";
        Package p8 = new Package();
        p8.packageId = 208;
        p8.destination = "Darjeeling";
        p8.price = 25000;
        t8.pkg = p8;
        t8.displayTourismInfo();

        TourismAngadi t9 = new TourismAngadi();
        t9.tourismId = 109;
        t9.touristName = "Manoj";
        t9.email = "manoj@gmail.com";
        Package p9 = new Package();
        p9.packageId = 209;
        p9.destination = "Andaman";
        p9.price = 35000;
        t9.pkg = p9;
        t9.displayTourismInfo();

        TourismAngadi t10 = new TourismAngadi();
        t10.tourismId = 110;
        t10.touristName = "Pooja";
        t10.email = "pooja@gmail.com";
        Package p10 = new Package();
        p10.packageId = 210;
        p10.destination = "Jaipur";
        p10.price = 14000;
        t10.pkg = p10;
        t10.displayTourismInfo();

        TourismAngadi t11 = new TourismAngadi();
        t11.tourismId = 111;
        t11.touristName = "Sanjay";
        t11.email = "sanjay@gmail.com";
        Package p11 = new Package();
        p11.packageId = 211;
        p11.destination = "Agra";
        p11.price = 13000;
        t11.pkg = p11;
        t11.displayTourismInfo();

        TourismAngadi t12 = new TourismAngadi();
        t12.tourismId = 112;
        t12.touristName = "Divya";
        t12.email = "divya@gmail.com";
        Package p12 = new Package();
        p12.packageId = 212;
        p12.destination = "Shimla";
        p12.price = 21000;
        t12.pkg = p12;
        t12.displayTourismInfo();

        TourismAngadi t13 = new TourismAngadi();
        t13.tourismId = 113;
        t13.touristName = "Akash";
        t13.email = "akash@gmail.com";
        Package p13 = new Package();
        p13.packageId = 213;
        p13.destination = "Ladakh";
        p13.price = 40000;
        t13.pkg = p13;
        t13.displayTourismInfo();

        TourismAngadi t14 = new TourismAngadi();
        t14.tourismId = 114;
        t14.touristName = "Lakshmi";
        t14.email = "lakshmi@gmail.com";
        Package p14 = new Package();
        p14.packageId = 214;
        p14.destination = "Udaipur";
        p14.price = 17000;
        t14.pkg = p14;
        t14.displayTourismInfo();

        TourismAngadi t15 = new TourismAngadi();
        t15.tourismId = 115;
        t15.touristName = "Rohit";
        t15.email = "rohit@gmail.com";
        Package p15 = new Package();
        p15.packageId = 215;
        p15.destination = "Hyderabad";
        p15.price = 11000;
        t15.pkg = p15;
        t15.displayTourismInfo();

        TourismAngadi t16 = new TourismAngadi();
        t16.tourismId = 116;
        t16.touristName = "Neha";
        t16.email = "neha@gmail.com";
        Package p16 = new Package();
        p16.packageId = 216;
        p16.destination = "Pondicherry";
        p16.price = 16000;
        t16.pkg = p16;
        t16.displayTourismInfo();

        TourismAngadi t17 = new TourismAngadi();
        t17.tourismId = 117;
        t17.touristName = "Karthik";
        t17.email = "karthik@gmail.com";
        Package p17 = new Package();
        p17.packageId = 217;
        p17.destination = "Rishikesh";
        p17.price = 20000;
        t17.pkg = p17;
        t17.displayTourismInfo();

        TourismAngadi t18 = new TourismAngadi();
        t18.tourismId = 118;
        t18.touristName = "Deepa";
        t18.email = "deepa@gmail.com";
        Package p18 = new Package();
        p18.packageId = 218;
        p18.destination = "Varanasi";
        p18.price = 15000;
        t18.pkg = p18;
        t18.displayTourismInfo();

        TourismAngadi t19 = new TourismAngadi();
        t19.tourismId = 119;
        t19.touristName = "Vikram";
        t19.email = "vikram@gmail.com";
        Package p19 = new Package();
        p19.packageId = 219;
        p19.destination = "Delhi";
        p19.price = 12000;
        t19.pkg = p19;
        t19.displayTourismInfo();

        TourismAngadi t20 = new TourismAngadi();
        t20.tourismId = 120;
        t20.touristName = "Anil";
        t20.email = "anil@gmail.com";
        Package p20 = new Package();
        p20.packageId = 220;
        p20.destination = "Mumbai";
        p20.price = 14000;
        t20.pkg = p20;
        t20.displayTourismInfo();*/

        Package p1 = new Package(201, "Goa", 15000);
        TourismAngadi t1 = new TourismAngadi(101, "Rahul", "rahul@gmail.com", p1);
        t1.displayTourismInfo();

        Package p2 = new Package(202, "Kerala", 18000);
        TourismAngadi t2 = new TourismAngadi(102, "Anita", "anita@gmail.com", p2);
        t2.displayTourismInfo();

        Package p3 = new Package(203, "Manali", 22000);
        TourismAngadi t3 = new TourismAngadi(103, "Kiran", "kiran@gmail.com", p3);
        t3.displayTourismInfo();

        Package p4 = new Package(204, "Ooty", 12000);
        TourismAngadi t4 = new TourismAngadi(104, "Sneha", "sneha@gmail.com", p4);
        t4.displayTourismInfo();

        Package p5 = new Package(205, "Kashmir", 30000);
        TourismAngadi t5 = new TourismAngadi(105, "Arjun", "arjun@gmail.com", p5);
        t5.displayTourismInfo();

        Package p6 = new Package(206, "Coorg", 10000);
        TourismAngadi t6 = new TourismAngadi(106, "Meera", "meera@gmail.com", p6);
        t6.displayTourismInfo();

        Package p7 = new Package(207, "Mysore", 9000);
        TourismAngadi t7 = new TourismAngadi(107, "Ravi", "ravi@gmail.com", p7);
        t7.displayTourismInfo();

        Package p8 = new Package(208, "Darjeeling", 25000);
        TourismAngadi t8 = new TourismAngadi(108, "Priya", "priya@gmail.com", p8);
        t8.displayTourismInfo();

        Package p9 = new Package(209, "Andaman", 35000);
        TourismAngadi t9 = new TourismAngadi(109, "Manoj", "manoj@gmail.com", p9);
        t9.displayTourismInfo();

        Package p10 = new Package(210, "Jaipur", 14000);
        TourismAngadi t10 = new TourismAngadi(110, "Pooja", "pooja@gmail.com", p10);
        t10.displayTourismInfo();

        Package p11 = new Package(211, "Agra", 13000);
        TourismAngadi t11 = new TourismAngadi(111, "Sanjay", "sanjay@gmail.com", p11);
        t11.displayTourismInfo();

        Package p12 = new Package(212, "Shimla", 21000);
        TourismAngadi t12 = new TourismAngadi(112, "Divya", "divya@gmail.com", p12);
        t12.displayTourismInfo();

        Package p13 = new Package(213, "Ladakh", 40000);
        TourismAngadi t13 = new TourismAngadi(113, "Akash", "akash@gmail.com", p13);
        t13.displayTourismInfo();

        Package p14 = new Package(214, "Udaipur", 17000);
        TourismAngadi t14 = new TourismAngadi(114, "Lakshmi", "lakshmi@gmail.com", p14);
        t14.displayTourismInfo();

        Package p15 = new Package(215, "Hyderabad", 11000);
        TourismAngadi t15 = new TourismAngadi(115, "Rohit", "rohit@gmail.com", p15);
        t15.displayTourismInfo();

        Package p16 = new Package(216, "Pondicherry", 16000);
        TourismAngadi t16 = new TourismAngadi(116, "Neha", "neha@gmail.com", p16);
        t16.displayTourismInfo();

        Package p17 = new Package(217, "Rishikesh", 20000);
        TourismAngadi t17 = new TourismAngadi(117, "Karthik", "karthik@gmail.com", p17);
        t17.displayTourismInfo();

        Package p18 = new Package(218, "Varanasi", 15000);
        TourismAngadi t18 = new TourismAngadi(118, "Deepa", "deepa@gmail.com", p18);
        t18.displayTourismInfo();

        Package p19 = new Package(219, "Delhi", 12000);
        TourismAngadi t19 = new TourismAngadi(119, "Vikram", "vikram@gmail.com", p19);
        t19.displayTourismInfo();

        Package p20 = new Package(220, "Mumbai", 14000);
        TourismAngadi t20 = new TourismAngadi(120, "Anil", "anil@gmail.com", p20);
        t20.displayTourismInfo();

        
    }
}
